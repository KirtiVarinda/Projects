...
public class DinnerFragment extends Fragment {
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        textView=(TextView)view.findViewById(R.id.textView113);
        error=(LinearLayout)view.findViewById(R.id.error1);
...
//
//
//        System.out.println("12 time start" + TimeManager.CurrenttwelevFormatTime(time1[0], time1[1]));
//        System.out.println("12 time start" + TimeManager.CurrenttwelevFormatTime(time2[0], time2[1]));
//
//
//
        ...
        if ( currentTime < endTime ) {
            ...
            getActivity().runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    textView.setText("We deliver Dinner till " + eTime + " and last order time is " + before);
        ...
        if(runSuccess) {
            ...
            boolean check;
            if(ProductPage.checkDayIsToday){
                check=ProductPage.todayTiffinDataDinner!= null;
                tiffinData=ProductPage.todayTiffinDataDinner;

            }else{
                check=ProductPage.tomorrowTiffinDataDinner != null;
                tiffinData=ProductPage.tomorrowTiffinDataDinner;
            }
            if (check) {
                setDinnerListAdapter(tiffinData);
                ...
                if (reponse.equals(ServerSync.DEFAULT_RESPONSE_VALUE) || reponse.equals(ServerSync.HTTP_ERROR)) {
                    ...
                    if (parseJsonData.checkStatus(reponse, "status").equals("success")) {
                        ...
                        if(ProductPage.checkDayIsToday) {
                            TiffinData[] tiffiDataDinnerToday  = parseJsonData.parseTiffinJson(reponse, tiffinCategories[index].getCategoryId(), tiffinCategories[index].getCategoryName(),tiffinCategories[index].getCategoryStartTime()+"-l-"+tiffinCategories[index].getCategoryEndTime());       // get categories from json
                            TiffinData[] addonsDinnerToday  = parseJsonData.parseTiffinJson(ProductPage.addonResponse, tiffinCategories[index].getCategoryId(), tiffinCategories[index].getCategoryName()+"_addons",tiffinCategories[index].getCategoryStartTime()+"-l-"+tiffinCategories[index].getCategoryEndTime());       // get categories from json

                            ProductPage.todayTiffinDataDinner = MergeData.mergeTwoBeans(tiffiDataDinnerToday, addonsDinnerToday);
                            setDinnerListAdapter(ProductPage.todayTiffinDataDinner);
                        }else{

                            TiffinData[] tiffiDataDinnerTomorrow  = parseJsonData.parseTiffinJson(reponse, tiffinCategories[index].getCategoryId(), tiffinCategories[index].getCategoryName(),tiffinCategories[index].getCategoryStartTime()+"-l-"+tiffinCategories[index].getCategoryEndTime());       // get categories from json
                            TiffinData[] addonsDinnerTomorrow   = parseJsonData.parseTiffinJson(ProductPage.addonResponse, tiffinCategories[index].getCategoryId(), tiffinCategories[index].getCategoryName()+"_addons",tiffinCategories[index].getCategoryStartTime()+"-l-"+tiffinCategories[index].getCategoryEndTime());       // get categories from json

                            ProductPage.tomorrowTiffinDataDinner = MergeData.mergeTwoBeans(tiffiDataDinnerTomorrow, addonsDinnerTomorrow);


                            setDinnerListAdapter(ProductPage.tomorrowTiffinDataDinner);
                        }
    ...
    private void setDinnerListAdapter(TiffinData[] tiffinData) {
