...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	boolean swipingEnd=false;
	int lastOpenedPosition;
	String imageType;
	float hX;
	float hY;
	float dX;
	...
	public SwipeListViewTouchListener(){
		screenResolution=MainReminder.width;
		onePartofReso=screenResolution/7;
	}
	...
	public void setLastOpenPosition(int position){
	 ...
	 */
	  ...
	  */
        ...
        return new AbsListView.OnScrollListener() {
	     ...
	     void closeOpenedItem(int position){
			 if (opened != null) {
				 if (opened.get(position)) {
					 closeAnimate(swipeListView.getChildAt(position).findViewById(swipeFrontView), position);
				 }
			 }
		 }
	 /**
	...
	//	 float f=swipeListView.getOffset1();                               //for overscroll
	//	 float f1=swipeListView.getOffset2();                               //for overscroll
	//	 if(mLastTouchX - motionEvent.getY() != 0.0F)                          //for overscroll
	//		 mScrollX = mLastTouchX - motionEvent.getY();                     //for overscroll
		 ...
		 case MotionEvent.ACTION_DOWN: {
			 ...
			 firstDownX=(int)motionEvent.getX();
			 counter=1;
		 ...
		 case MotionEvent.ACTION_UP: {
			 SwipeListView.overScrollCheck=false;
			  if(MainReminder.imageCatScroll){
				
				
				 MainReminder.catAlarm(imageType);
				 MainReminder.imageCatScroll=false;
			 } 
			 float deltaX = motionEvent.getRawX() - downX;
			/* if (Build.VERSION.SDK_INT > Build.VERSION_CODES.GINGERBREAD_MR1) {                 //for overscroll
			 if(isstretch){
				 isOverScroll=false;
				 float f4 = touchDownY - (motionEvent.getY());
				 if(f == 0.0F || f == f1)
					 swipeListView.createSnapBackAnimationForOffset(f4);
				 touchDownY = -1F;
				 isstretch=false;
				 return false;
			 }
		 ...
		 case MotionEvent.ACTION_MOVE: {
			 ...
			 dX=touchUpX-touchDownX;
			 dY=touchUpY-touchDownY;
			 if(dY>Math.abs(dX) && SwipeListView.overScrollCheck){
				 MainReminder.animateText();
			 ...
			 if(MainReminder.imageCatScroll){
				 ...
				 if(hX>=0 &&hX<onePartofReso){
					 imageType="all";
					 MainReminder.changeImage(imageType);
				 }else if(hX>=onePartofReso && hX<onePartofReso*2){
					 imageType="text";
					 MainReminder.changeImage(imageType);
				 }else if(hX>=onePartofReso*2 && hX<onePartofReso*3){
					 imageType="email";
					 MainReminder.changeImage(imageType);
				 }else if(hX>=onePartofReso*3 && hX<onePartofReso*4){
					 imageType="call";
					 MainReminder.changeImage(imageType);
				 }else if(hX>=onePartofReso*4 && hX<onePartofReso*5){
					 imageType="event";
					 MainReminder.changeImage(imageType);
				 }else if(hX>=onePartofReso*5 && hX<onePartofReso*6){
					 imageType="todo";
					 MainReminder.changeImage(imageType);
				 }else if(hX>=onePartofReso*6 && hX<onePartofReso*7){
					 imageType="WakeUp";
			 ...
			 float swipeDeltaX=touchUpX-touchDownX;
			 float swipeDeltaY=touchUpY-touchDownY;
			 velocityTracker.addMovement(motionEvent);
			 velocityTracker.computeCurrentVelocity(1000);
			 float velocityX = Math.abs(velocityTracker.getXVelocity());
			 float velocityY = Math.abs(velocityTracker.getYVelocity());
			 float deltaX = motionEvent.getRawX() - downX;
			 float deltaX1 = motionEvent.getRawX();
			 float deltaY1 = motionEvent.getRawY()- downY;
			 float deltaMode = Math.abs(deltaX);
			 //      Log.d("Positions","LastDown"+lastDownX );

			 if(firstDownX<viewWidth/4 && deltaX1>=viewWidth/2 && !swiping && deltaY1<50 ){
				 if(counter==1){
					 swipeListView.onDone(downPosition);
					 counter+=1;
				 }
			 }
			 ...
			 if (swiping && downPosition != ListView.INVALID_POSITION && !SwipeListView.overScrollCheck ) {
				 if (opened.get(downPosition)) {
			...
			/* if (Build.VERSION.SDK_INT > Build.VERSION_CODES.GINGERBREAD_MR1) {                 //for overscroll
			 ...
			 if(Math.abs(swipeDeltaY)>Math.abs(swipeDeltaX)){
				if(f == 0.0F && touchDownY < motionEvent.getY())
			 {
				 isOverScroll=true;
				 if(touchDownY == -1F)
					 touchDownY = motionEvent.getY();
				 float f3 = touchDownY - (motionEvent.getY());
				 Log.d("IM","hahahah");
				 swipeListView.createAnimationForOffset(f3, f, f1);
				 //         if(downPosition==0 && swipeDeltaY>30 ){
				 //         	  MainReminder.showTextView();
				 //         	  }
				 isstretch=true;
				 return true;
			 }
			 ...
			 if((int)f == (int)f1 && touchDownY > motionEvent.getY())
			 {
				 ...
				 if(touchDownY == -1F)
					 touchDownY = motionEvent.getY();
				 float f2 = touchDownY - (motionEvent.getY());
				 Log.i("IM", "K");
				 swipeListView.createAnimationForOffset(f2, f, f1);
				 isstretch=true;
				 return true;
			 } else
			 {
