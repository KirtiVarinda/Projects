...
public class GLFactory {
    ...
    public Obj newTree(Vec pos) {
        Obj tree = new Obj();
        MeshComponent g1 = new Shape(null, pos);
        tree.setComp(g1);
        g1.addChild(this.newTreangle(Color.red()));
        MeshComponent g2 = new Shape(Color.green(), new Vec(0, 5, 0.1f));
        g1.addChild(g2);

        g2.addChild(this.newCircle(null));
        MeshComponent g3 = new Shape(Color.green(), new Vec(0, 4, 0));
        g2.addChild(g3);

        g3.addChild(this.newCircle(null));
        MeshComponent g4 = new Shape(Color.green(), new Vec(0, 2, 0));
        g3.addChild(g4);
        g4.addChild(this.newCircle(null));
    ...
    public Obj newTrees(Vec position) {
        System.out.println("hareeeeeeekrishnaaaaaaa");

        Obj treeSystem = new Obj();
        MeshComponent treeBox = new Shape();
        System.out.println("position..." + position);
        if (position != null)
            treeBox.setPosition(position);
        treeSystem.setComp(treeBox);

        MeshComponent circle1 = new Shape();
        MeshComponent circle2 = new Shape();
        circle1.addChild(circle2);

        MeshComponent base = GLFactory.getInstance().newTreangle(Color.red());
        GLFactory.getInstance().addRotateAnimation(base, 30, new Vec(1, 1, 1));
        treeBox.addChild(base);

        GLFactory.getInstance().addRotateAnimation(circle1, 40,
                new Vec(0.5f, 0.3f, 1));
        circle2.setPosition(new Vec(3, 0, 0));
        treeBox.addChild(circle1);

        MeshComponent earth = GLFactory.getInstance().newCircle(Color.green());
        earth.scaleEqual(0.5f);
        circle2.addChild(earth);

        MeshComponent moonring = new Shape();

        MeshComponent moon = GLFactory.getInstance().newCircle(Color.green());
